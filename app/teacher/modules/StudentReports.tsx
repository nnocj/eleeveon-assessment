...
/**
 * app/teacher/modules/StudentReports.tsx
 * ---------------------------------------------------------
 * TEACHER STUDENT REPORTS — ASSIGNED CLASS ONLY
 * ---------------------------------------------------------
 *
 * No branch selector.
 *
...
export default function TeacherStudentReports() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const assignedClassIds = new Set(
        assignedClassTeacherRows.map((row: any) => idOf(row.classId)).filter(Boolean),
      );

      const assignedEnrollmentRows = enrollmentRows.filter(
        (row: any) =>
          sameTenant(row) &&
          assignedClassIds.has(idOf(row.classId)),
      );

      const assignedStudentIds = new Set(
        assignedEnrollmentRows.map((row: any) => idOf(row.studentId)).filter(Boolean),
      );

      const assignedClassSubjectRows = classSubjectRows.filter(
        (row: any) =>
          sameTenant(row) &&
          row.active !== false &&
          assignedClassIds.has(idOf(row.classId)),
      ...
      const assignedClassSubjectIds = new Set(
        assignedClassSubjectRows.map((row: any) => idOf(row.id)).filter(Boolean),
      );

      const assignedReportCards = scopedReportCards.filter(
        (row: any) =>
          assignedClassIds.has(idOf(row.classId)) ||
      ...
      const scopedStudentsWithMedia = await Promise.all(
        studentRows
          .filter(
            (student: any) =>
              sameTenant(student) && assignedStudentIds.has(idOf(student.id)),
          )
      ...
      const branchReportCardIds = new Set<string>(
        assignedReportCards.map((row) => idOf(row.id)).filter(Boolean),
      );
          ...
          (row: any) =>
            ...
            (idOf(row.id) === teacherId ||
              assignedClassTeacherRows.some(
                (assignment: any) => idOf(assignment.teacherId) === idOf(row.id),
              )),
        ),
      );
      setClasses(
      ...
      setClassSubjects(assignedClassSubjectRows);
      setStudentParents(studentParentRows.filter(sameTenant));
      setStudentEnrollments(assignedEnrollmentRows);
      ...
      setAssessmentEntries(
        entryRows.filter(
          (row: any) =>
            sameTenant(row) &&
            (assignedClassSubjectIds.has(idOf(row.classSubjectId)) ||
              assignedStudentIds.has(idOf(row.studentId))),
        ),
      );
      setGradingStructures(
      ...
      setReportCards(assignedReportCards);
  ...
  if (accountLoading || contextLoading || settingsLoading || pageLoading) {
    return (
      <State
        primary={primary}
        title="Opening Teacher Student Reports..."
        text="Checking your teacher profile, assigned class, reports, results and attendance records."
      />
    );
  }
  ...
  if (!isTeacherMembership) {
    return (
      <State
        ...
        text="Student reports on this page are available only from a teacher membership."
      />
    );
  }
  ...
  if (!teacherId) {
    return (
      <State
        ...
        text="Your membership must be linked to a permanent teacher profile before student reports can be opened."
      />
    );
  }
  ...
  if (!classTeachers.length) {
    return (
      <State
        ...
        text="You can open full student reports only for a class where you are the assigned class teacher."
      />
    );
  }
