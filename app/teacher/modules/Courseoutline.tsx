...
/**
 * app/teacher/modules/CourseOutline.tsx
 * Eleeveon Teacher Course Outline V1.
...
export default function TeacherCourseOutline() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const assignedClassSubjects = (classSubjectRows as ClassSubject[]).filter(
        (row: any) =>
      ...
      const assignedClassIds = new Set(
        assignedClassSubjects.map((row: any) => idOf(row.classId)).filter(Boolean),
      );
      const assignedSubjectIds = new Set(
        assignedClassSubjects.map((row: any) => idOf(row.subjectId)).filter(Boolean),
      );
      const assignedCurriculumSubjectIds = new Set(
        assignedClassSubjects
          .map((row: any) => idOf(row.curriculumSubjectId))
          .filter(Boolean),
      );
        ...
        (curriculumSubjectRows as CurriculumSubject[])
          .filter(
            (row: any) =>
              sameTenant(row as TenantRow) &&
              isActiveRow(row) &&
              assignedCurriculumSubjectIds.has(idOf(row.id)),
          )
          .sort(
            (a: any, b: any) =>
        ...
        (subjectRows as Subject[])
          .filter(
            (row: any) =>
              sameTenant(row as TenantRow) &&
              isActiveRow(row) &&
              assignedSubjectIds.has(idOf(row.id)),
          )
          .sort((a: any, b: any) =>
            String(a.name || "").localeCompare(String(b.name || "")),
          ),
      ...
      const assignedClassSubjectIds = new Set(
        assignedClassSubjects.map((row: any) => idOf(row.id)).filter(Boolean),
      );

      setAssessmentApplicabilities(
        (applicabilityRows as AssessmentApplicability[]).filter(
          (row: any) =>
            sameTenant(row as TenantRow) &&
  ...
  if (accountLoading || contextLoading || settingsLoading || loading) {
    return (
      <State
        primary={primary}
        title="Opening My Course Outlines..."
        text="Checking your teacher profile, assigned class subjects, curriculum and assessment readiness."
      />
    );
  }
  ...
  if (!isTeacherMembership) {
    return (
      <State
        ...
        text="Course outlines on this page are available only from a teacher membership."
      />
    );
  }
  ...
  if (!teacherId) {
    return (
      <State
        ...
        text="Your membership must be linked to a permanent teacher profile before assigned courses can be shown."
      />
    );
  }
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          ...
          <p>
            Course outlines are generated only from class subjects assigned to your teacher profile.
          </p>
          <button
            type="button"
  ...
  if (!classSubjects.length) {
    return (
      <State
        ...
        text="No active class subject is assigned to your teacher profile yet."
      />
    );
  }
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="My course outline search and actions"
      >
