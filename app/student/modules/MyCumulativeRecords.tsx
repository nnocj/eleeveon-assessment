...
/**
 * app/student/modules/MyCumulativeRecords.tsx
 * ---------------------------------------------------------
 * STUDENT PORTAL — MY CUMULATIVE RECORDS
 * ---------------------------------------------------------
 *
...
import CumulativeReportBook from "../../branch-admin/modules/reports/components/CumulativeReportBook";
import CumulativeTranscriptCard from "../../branch-admin/modules/reports/components/CumulativeTranscriptCard";
import { STUDENT_REPORT_TEMPLATE_REGISTRY } from "../../branch-admin/modules/reports/student-report-templates";
import { CUMULATIVE_TRANSCRIPT_TEMPLATE_REGISTRY } from "../../branch-admin/modules/reports/cumulative-transcript-templates";
import AnnualBroadsheet from "../../branch-admin/modules/reports/components/AnnualBroadsheet";
import PromotionSummary from "../../branch-admin/modules/reports/components/PromotionSummary";
import StudentProgressionTimeline from "../../branch-admin/modules/reports/components/StudentProgressionTimeline";
import ReportHeader from "../../branch-admin/modules/reports/components/ReportHeader";

import {
  buildCumulativeReportEngineOutput,
  buildStudentTranscript,
} from "../../branch-admin/modules/reports/engine/cumulative-report-engine";

import type {
  CumulativeReportEngineDataset,
  CumulativeReportFiltersState,
} from "../../branch-admin/modules/reports/engine/cumulative-report-types";
...
export default function MyCumulativeRecords() {
  ...
  const canViewCumulativeRecords = role === "student";
  ...
  const load = async () => {
    if (!authenticated || !accountId || !schoolId || !branchId || !canViewCumulativeRecords) {
      clearData();
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const scopedParents = parentRows.filter(
        (row) => sameTenant(row) || looseBranchTenant(row),
      );
      const scopedStudentParents = studentParentRows.filter(
        (row) => sameTenant(row) || looseBranchTenant(row),
      );
      ...
      const matchedStudent =
        (studentsWithMedia as Student[]).find(
          (row: any) =>
            membershipStudentId && sameId(row.id, membershipStudentId),
        ) ||
        (studentsWithMedia as Student[]).find((row: any) => {
      ...
      const portalStudents = (studentsWithMedia as Student[]).filter((row) =>
        allowedStudentIds.has(idOf(row.id)),
      );
      const portalStudentParents = scopedStudentParents.filter((row) =>
        allowedStudentIds.has(idOf(row.studentId)),
      );
      const portalParentIds = new Set(
        portalStudentParents.map((row) => idOf(row.parentId)).filter(Boolean),
      );
      const portalParents = scopedParents.filter((row) =>
        portalParentIds.has(idOf(row.id)),
      );
      ...
      setParents(portalParents);
      setStudentParents(portalStudentParents);
      setTeachers(
      ...
      setStudentReportSnapshots(
        snapshotRows.filter((row) => {
          if (!allowedStudentIds.has(idOf(row.studentId))) return false;
  ...
  useEffect(() => {
    setFilters((prev) => {
      ...
      return {
        ...
        studentId: membershipStudentId || prev.studentId,
        subjectId: branchChanged ? undefined : prev.subjectId,
      };
    });
  }, [branchId, currentAcademicStructureId, membershipStudentId]);
  ...
  if (accountLoading || contextLoading || settingsLoading || loading) {
    return (
      <State
        primary={primary}
        title="Opening My Cumulative Records..."
        text="Checking your historical reports, promotions and academic progression."
      />
    );
  }
  ...
  if (!authenticated || !accountId) {
    return (
      <State
        ...
        text="You must sign in before viewing your cumulative records."
      />
    );
  }

  if (!canViewCumulativeRecords) {
    return (
      <State
        primary={primary}
  ...
  if (!membershipStudentId && students.length === 0) {
    return (
      <State
        primary={primary}
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          ...
          <p>
            Your cumulative records require an active school and branch workspace.
          </p>
          <button
            type="button"
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="My cumulative records search and actions"
      >
        <label className="ba-search">
          ...
          <input
            ...
            placeholder="Search my cumulative records..."
            aria-label="Search my cumulative records"
          />
        </label>
      ...
      <section className="ba-sheet">
        <div className="ba-sheet-head">
          <div>
            <h2>Filters</h2>
            <p>
              Choose the historical period, subject, or record type. Your student profile stays locked.
            </p>
          </div>
      ...
      <section className="ba-sheet small">
        ...
        <div className="ba-menu-list">
          ...
          <button type="button" onClick={onRefresh}>
            <span>↻</span>
            <b>Refresh</b>
            <small>Reload my cumulative records</small>
          </button>
        </div>
      </section>
    </div>
