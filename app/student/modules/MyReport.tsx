...
/**
 * app/student/modules/Myreport.tsx
 * ---------------------------------------------------------
...
import ReportAnalytics from "../../branch-admin/modules/reports/components/ReportAnalytics";
import StudentReportCard from "../../branch-admin/modules/reports/components/StudentReportCard";
...
} from "../../branch-admin/modules/reports/shared/ReportTemplateTypes";
...
export default function MyReport() {
  ...
  const membership = useMemo(
    () =>
      (openWorkspace?.membership as Record<string, any> | null) ||
      (activeMembership as Record<string, any> | null) ||
      readStoredActiveMembership() ||
      {},
    [activeMembership, openWorkspace?.membership],
  );

  const membershipStudentId = firstLocalId(
    membership?.studentId,
    membership?.studentLocalId,
    membership?.student?.id,
    openWorkspace?.studentId,
    safeStorageRead("activeStudentId"),
  );

  const role = String(membership?.role || openWorkspace?.role || "").toLowerCase();
  const canViewStudentReport = role === "student";
  ...
  const { loading: pageLoading, setLoading: setPageLoading } =
    useBackgroundLoader();
  const [mode, setMode] = useState<ReportMode>("student-report"); // Student portal is locked to one report.
  ...
  const load = async () => {
    if (!authenticated || !accountId || !schoolId || !branchId || !canViewStudentReport) {
      clearState();
    ...
    try {
      ...
      const matchedStudent =
        (scopedStudentsWithMedia as Student[]).find(
          (row: any) =>
            membershipStudentId && idOf(row.id) === membershipStudentId,
        ) ||
        (scopedStudentsWithMedia as Student[]).find((row: any) => {
      ...
      const resolvedStudentId = firstLocalId(
        membershipStudentId,
        matchedStudent?.id,
      );
      ...
      const branchReportCardIds = new Set<string>(
        scopedReportCards
          .filter((row: any) =>
            allowedStudentIds.has(idOf(row.studentId)),
          )
          .map((row) => idOf(row.id))
          .filter(Boolean),
      );

      const studentRowsForPortal = (scopedStudentsWithMedia as Student[]).filter(
        (student: any) => allowedStudentIds.has(idOf(student.id)),
      );

      const belongsToActiveStudent = (row: any) =>
        allowedStudentIds.has(idOf(row.studentId));

      const activeStudentParentLinks = scopedStudentParents.filter((row: any) =>
        allowedStudentIds.has(idOf(row.studentId)),
      );

      const activeParentIds = new Set(
        activeStudentParentLinks
          .map((row: any) => idOf(row.parentId))
          .filter(Boolean),
      );

      const activeStudentParents = scopedParents.filter((row: any) =>
        activeParentIds.has(idOf(row.id)),
      );
      ...
      setStudents(studentRowsForPortal);
      setFilters((prev) => ({
        ...prev,
        studentId: resolvedStudentId || undefined,
      }));
      setParents(activeStudentParents);
      ...
      setStudentParents(activeStudentParentLinks);
      setStudentEnrollments(
        enrollmentRows.filter(
          (row) => sameTenant(row) && belongsToActiveStudent(row),
        ),
      );
      ...
      setAssessmentEntries(entryRows.filter((row) => sameTenant(row) && belongsToActiveStudent(row)));
      setGradingStructures(
      ...
      setAttendance(attendanceRows.filter((row) => sameTenant(row) && belongsToActiveStudent(row)));
      setStudentAttendanceSummaries(
        (studentAttendanceSummaryRows as StudentAttendanceSummary[]).filter(
          (row: any) => sameTenant(row) && belongsToActiveStudent(row),
        ),
      );
      setComputedResults(computedRows.filter((row) => sameTenant(row) && belongsToActiveStudent(row)));
      setReportCards(scopedReportCards.filter(belongsToActiveStudent));
  ...
  }, [authenticated, accountId, schoolId, branchId, dataRevision, canViewStudentReport]);
  ...
  useEffect(() => {
    if (filters.studentId || filteredStudents.length === 0) return;
    setFilters((prev) => ({
      ...prev,
      studentId: idOf(filteredStudents[0]?.id) || undefined,
      classId:
        prev.classId ||
        idOf(
          studentEnrollments.find(
            (row: any) =>
              idOf(row.studentId) === idOf(filteredStudents[0]?.id) &&
              ["active", "promoted"].includes(
  ...
  }, [filteredStudents, filters.studentId, studentEnrollments]);
    ...
    if (!hasCoreSetup) {
      return (
        <Empty
          ...
          text="Your school has not published enough academic data to build your report yet."
        />
      );
    }
  ...
  if (accountLoading || contextLoading || settingsLoading || pageLoading) {
    return (
      <State
        primary={primary}
        title="Opening My Report..."
        text="Checking your academic records, report template and attendance."
      />
    );
  }
  ...
  if (!authenticated || !accountId) {
    return (
      <State
        ...
        text="You must sign in before opening your report."
      />
    );
  }
  ...
  if (!canViewStudentReport) {
    return (
      <State
        ...
        text="Open this page from an active student-role workspace."
      />
    );
  }

  if (!membershipStudentId && students.length === 0 && !pageLoading) {
    return (
      <State
        primary={primary}
        title="Student profile not linked"
        text="The active student membership needs a studentId, studentLocalId, or matching student contact."
      />
    );
  }
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          ...
          <p>
            Your report is generated inside the selected student
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="My report search and actions"
      >
        <label className="ba-search">
          ...
          <input
            ...
            placeholder="Search my report..."
            aria-label="Search my report"
          />
        </label>
      ...
      <section className="ba-sheet">
        <div className="ba-sheet-head">
          <div>
            <h2>Filters</h2>
            <p>
              Choose the academic period or subject. Your student profile stays locked to the assigned
              branch.
            </p>
          </div>
        ...
        <div className="ba-form compact">
          <label>
            ...
            >
              <option value="student-report">My Report</option>
          ...
          {mode === "student-report" && (
            <label>
            <span>Student</span>
            <select value={filters.studentId || ""} disabled>
              {filteredStudents.map((item: any) => (
                <option key={item.id} value={item.id}>
                  {item.fullName || item.name || "Student"}
                </option>
              ))}
            </select>
          </label>
...
/* Student My Report golden additions */
