...
/**
 * app/school-admin/modules/SchoolCalendar.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON SCHOOL ADMIN — SCHOOL CALENDAR
 * --------------------------------------------------------------------------
 * School-scoped calendar overview across every branch assigned to the school.
 *
 * Features:
 * - stable string/UUID IDs;
 * - reads calendarEvents directly from the offline-first Dexie store;
 * - account + school + valid-branch scoping;
 * - Month, Week, Day and Year calendar modes;
 * - multi-view calendar, cards, table and analytics;
 * - branch filtering and responsive dark-mode-safe UI;
 * - cancelled and soft-deleted events are excluded.
 */
...
function isSchoolRow(row: AnyRow, accountId?: string | null, schoolId?: string | null) {
  return (
    row &&
    row.isDeleted !== true &&
...
function branchName(branches: AnyRow[], branchId: any) {
  const branch = branches.find((item: AnyRow) => sameId(item.id || item.localId, branchId));
...
export default function SchoolCalendar() {
  ...
  async function load() {
    ...
    try {
      ...
      const branchIds = new Set(
        branchRows
          .map((branch: AnyRow) => idOf(branch.id || branch.localId))
          .filter(Boolean),
      );

      const allEvents = (await safeArray<AnyRow>("calendarEvents"))
        .filter((event: AnyRow) => isSchoolRow(event, accountId, schoolId))
        .filter((event: AnyRow) => {
          const eventBranchId = idOf(event.branchId);
          return !eventBranchId || branchIds.has(eventBranchId);
        });
  ...
  const filtered = useMemo(() => {
    ...
    return events
      .filter((event: AnyRow) => branchId === "all" || sameId(event.branchId, branchId))
      .filter((event: AnyRow) => {
        if (!q) return true;
    ...
    return (
      <main className="sa-page" style={{ "--sa-primary": primary } as React.CSSProperties}>
        ...
        <section className="sa-state">
          ...
          <p>Loading events across the assigned school and its branches.</p>
        </section>
      </main>
    );
  }
  ...
  return (
    <main className="sa-page" style={{ "--sa-primary": primary } as React.CSSProperties}>
      ...
      <section className="sa-hero">
        <div className="sa-hero-left">
          ...
          <div className="sa-title">
            ...
            <span>{activeSchool?.name || "School"} · All branches · Calendar, cards, table and analytics</span>
          </div>
        </div>
      ...
      <section className="sa-filter">
        <input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search school events..." />
      ...
      {view === "analytics" ? (
        <section className="sa-section sa-breakdown-grid">
          {branches.map((branch: AnyRow) => {
            const count = events.filter((event: AnyRow) => sameId(event.branchId, branch.id || branch.localId)).length;
