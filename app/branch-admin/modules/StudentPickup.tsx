...
/**
 * app/branch-admin/modules/StudentPickup.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON Student Pickup — PHASE 10
 *
...
const PRIMARY_TABLE = "pickupAuthorizations";
const READ_ONLY = false;
const SOURCE_TABLES = [{"table": "pickupAuthorizations", "label": "Authorization"}, {"table": "studentPickupEvents", "label": "Pickup Event"}];
const FIELD_CONFIG = [{"key": "studentId", "label": "Student ID", "type": "text", "options": []}, {"key": "authorizedPersonType", "label": "Person Type", "type": "select", "options": ["parent", "guardian", "other"]}, {"key": "fullName", "label": "Collector Name", "type": "text", "options": []}, {"key": "phone", "label": "Phone", "type": "text", "options": []}, {"key": "relationship", "label": "Relationship", "type": "text", "options": []}, {"key": "status", "label": "Status", "type": "select", "options": ["pending", "approved", "rejected", "revoked", "expired"]}, {"key": "validFrom", "label": "Valid From", "type": "datetime-local", "options": []}, {"key": "validUntil", "label": "Valid Until", "type": "datetime-local", "options": []}, {"key": "recurring", "label": "Recurring", "type": "checkbox", "options": []}, {"key": "note", "label": "Note", "type": "textarea", "options": []}];
const STATUS_KEYS = ["approved", "pending", "rejected", "revoked", "expired"];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["student_pickup.view","student_pickup.read","student_pickup.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["student_pickup.manage","student_pickup.write"].includes(p)));
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"Pickup Authorization updated successfully.":"Pickup Authorization created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save Pickup Authorization.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","Pickup Authorization removed.")}catch(error){console.error(error);notify("error","Failed to remove Pickup Authorization.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening Student Pickup..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view Student Pickup."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search student pickup..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>♙</i><h3>No records found</h3><p>No student pickup match the current search and filters.</p></section>:null}
