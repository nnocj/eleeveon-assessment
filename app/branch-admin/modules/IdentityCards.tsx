...
/**
 * app/branch-admin/modules/IdentityCards.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON Student Identity Cards — PHASE 10
 *
...
const PRIMARY_TABLE = "studentIdentityCards";
const READ_ONLY = false;
const SOURCE_TABLES = [{"table": "studentIdentityCards", "label": "Card"}];
const FIELD_CONFIG = [{"key": "studentId", "label": "Student ID", "type": "text", "options": []}, {"key": "credentialId", "label": "Credential ID", "type": "text", "options": []}, {"key": "cardNumber", "label": "Card Number", "type": "text", "options": []}, {"key": "status", "label": "Status", "type": "select", "options": ["draft", "issued", "printed", "active", "expired", "revoked", "replaced"]}, {"key": "issuedAt", "label": "Issued At", "type": "datetime-local", "options": []}, {"key": "expiresAt", "label": "Expires At", "type": "datetime-local", "options": []}];
const STATUS_KEYS = ["active", "issued", "printed", "draft", "expired"];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["identity_cards.view","identity_cards.read","identity_cards.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["identity_cards.manage","identity_cards.write"].includes(p)));
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"Identity Card updated successfully.":"Identity Card created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save Identity Card.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","Identity Card removed.")}catch(error){console.error(error);notify("error","Failed to remove Identity Card.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening Student Identity Cards..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view Student Identity Cards."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search student identity cards..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>▤</i><h3>No records found</h3><p>No student identity cards match the current search and filters.</p></section>:null}
