...
/**
 * reports/broadsheet-templates/ExecutiveBroadsheetTemplate.tsx
 * ---------------------------------------------------------
 * ELEEVEON SCHOOLS — EXECUTIVE BROADSHEET TEMPLATE
 * ---------------------------------------------------------
 *
 * One premium leadership-oriented renderer for:
 * - subject broadsheets
 * - class broadsheets
 ...
 * - premium performance panels
 * - formal leadership document
 * - high-contrast executive table headers
 * - clean management-review rows
 * - strong hierarchy for decision-making and review
...
import BroadsheetExecutiveHeader from "../shared/headers/broadsheets/BroadsheetExecutiveHeader";
...
function tableStyles(compact: boolean, primary: string): TableStyleSet {
  return {
    ...
    th: {
      ...
      padding: compact ? "4px 4px" : "5px 5px",
      background: `linear-gradient(180deg, ${primary}, ${primary}e8)`,
      color: "#fff",
      textAlign: "center",
      ...
      style={{
        ...
        gridTemplateColumns:
          settings.showBroadsheetStudentPhoto && photo
            ? `${compact ? 21 : 25}px minmax(0,1fr)`
            : "minmax(0,1fr)",
        gap: compact ? 4 : 5,
        alignItems: "center",
        minWidth: compact ? 132 : 155,
      }}
    >
      {settings.showBroadsheetStudentPhoto && photo && (
        <img
...
export default function ExecutiveBroadsheetTemplate({
  kind,
  dataset,
  header,
  template,
  ...
  return (
    <section
      className={`print-page report-page-break broadsheet-template-page broadsheet-executive-page ${className || ""}`}
      style={pageStyle}
    >
