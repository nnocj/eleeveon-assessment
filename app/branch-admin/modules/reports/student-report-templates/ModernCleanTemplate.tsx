...
/**
 * reports/student-report-templates/ModernCleanTemplate.tsx
 * ---------------------------------------------------------
 * ELEEVEON SCHOOLS — MODERN FRAMED STUDENT REPORT TEMPLATE
 * ---------------------------------------------------------
 *
 * Visual direction:
 * - fixed A4/PDF sheet with a modern double-frame border
 * - asymmetric ModernCleanHeader, not the classic formal layout
 * - student identity panel instead of many equal information boxes
 * - compact bordered result table with modern printable rows
 * - performance band for total / average / position / GPA
 * - remark cards, academic-period timeline and clean signature footer
 *
 * Data rule:
...
export default function ModernCleanTemplate({
  ...
  if (!dataset || !report || !header || !normalized) {
    return (
      <div className="src-empty-card modern-template-empty">
        <style>{css}</style>
  ...
  const reportPage = (
    <section
      className="print-page report-page-break student-report-card-page src-a4-page modern-clean-template-page"
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="modern-frame-line modern-frame-outer"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="modern-frame-line modern-frame-inner"
      />
      <div
        aria-hidden="true"
        className="modern-corner modern-corner-tl"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="modern-corner modern-corner-tr"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="modern-corner modern-corner-bl"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="modern-corner modern-corner-br"
        style={{ borderColor: primary }}
      />
...
.modern-frame-outer { inset: 4.5mm; border: 1.5px solid; opacity: .65; }
.modern-frame-inner { inset: 6.5mm; border: 1px solid #e5e7eb; }
.modern-corner {
  position: absolute;
  z-index: 0;
  width: 17mm;
...
.modern-corner-tl { left: 4.5mm; top: 4.5mm; border-left: 3px solid; border-top: 3px solid; border-top-left-radius: 18px; }
.modern-corner-tr { right: 4.5mm; top: 4.5mm; border-right: 3px solid; border-top: 3px solid; border-top-right-radius: 18px; }
.modern-corner-bl { left: 4.5mm; bottom: 4.5mm; border-left: 3px solid; border-bottom: 3px solid; border-bottom-left-radius: 18px; }
.modern-corner-br { right: 4.5mm; bottom: 4.5mm; border-right: 3px solid; border-bottom: 3px solid; border-bottom-right-radius: 18px; }
.modern-grade-pill {
  display: inline-flex;
  align-items: center;
  justify-content: center;
...
.modern-timeline-dot::before,
.modern-timeline-dot::after {
  content: "";
  position: absolute;
  top: 50%;
  width: 16px;
...
.modern-timeline-dot::before { right: 100%; }
.modern-timeline-dot::after { left: 100%; }
...
@media print {
  ...
  .modern-clean-template-page [data-report-color-block="true"] {
