...
/**
 * reports/student-report-templates/IBTemplate.tsx
 * ---------------------------------------------------------
 * ELEEVEON SCHOOLS — IB CLEAN INTERNATIONAL STUDENT REPORT TEMPLATE
 * ---------------------------------------------------------
 *
 * Visual direction:
 * - fixed A4/PDF sheet with an international academic / learner-profile identity
 * - compact IBHeader with structured metadata and restrained navy styling
...
export default function IBTemplate({
  dataset,
  template,
  settings,
  compact = false,
  ...
  if (!dataset || !report || !header || !normalized) {
    return (
      <div className="src-empty-card ib-template-empty">
        <style>{css}</style>
  ...
  const reportPage = (
    <section
      className="print-page report-page-break student-report-card-page src-a4-page ib-template-page"
...
@media print {
  ...
  .ib-template-page [data-report-color-block="true"] {
