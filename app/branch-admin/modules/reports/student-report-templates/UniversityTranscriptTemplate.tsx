...
/**
 * reports/student-report-templates/UniversityTranscriptTemplate.tsx
 * ---------------------------------------------------------
 * ELEEVEON SCHOOLS — UNIVERSITY TRANSCRIPT ACADEMIC RECORD TEMPLATE
 * ---------------------------------------------------------
 *
 * Visual direction:
 * - fixed A4/PDF sheet with an official registrar/transcript identity
 * - compact UniversityTranscriptHeader with official registrar/transcript styling
 * - formal student record panel with optional photo and precise details
 * - dense transcript table, academic summary and certification comments
...
export default function UniversityTranscriptTemplate({
  dataset,
  template,
  settings,
  compact = false,
    ...
    mergeStudentReportTemplateSettings(
      {
        ...DEFAULT_STUDENT_REPORT_TEMPLATE_SETTINGS,
        ...(settings || {}),
        templateCode: "university_transcript",
        layoutKey: "university_transcript",
        templateName: "University Transcript",
  ...
  if (!dataset || !report || !header || !normalized) {
    return (
      <div className="src-empty-card university-transcript-template-empty">
        <style>{css}</style>
  ...
  const reportPage = (
    <section
      className="print-page report-page-break student-report-card-page src-a4-page university-transcript-template-page"
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="university-transcript-frame-line university-transcript-frame-outer"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="university-transcript-frame-line university-transcript-frame-inner"
      />
      <div
        aria-hidden="true"
        className="university-transcript-corner university-transcript-corner-tl"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="university-transcript-corner university-transcript-corner-tr"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="university-transcript-corner university-transcript-corner-bl"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="university-transcript-corner university-transcript-corner-br"
        style={{ borderColor: primary }}
      />

      <div style={{ position: "relative", zIndex: 1 }}>
        <UniversityTranscriptHeader
          header={header}
          dataset={dataset}
          settings={resolvedSettings}
          ...
          >
            ...
            >
              <div style={{ minWidth: 0 }}>
                <div style={smallLabel}>Student Record</div>
                <div
                  style={{
                    marginTop: 1,
          ...
          {resolvedSettings.showStudentPhoto && (
            ...
            >
              {studentPhoto ? (
                ...
                <span
                  style={{ fontSize: 9, fontWeight: 950, color: "#4b5563" }}
                >
                  PHOTO
                </span>
              )}
            ...
            <tbody>
                ...
                <tr key={subject.classSubjectId}>
                  ...
                  {resolvedSettings.showGrade && (
                    <td style={{ ...tableStyles.td, textAlign: "center" }}>
                      <span
                        className="university-transcript-grade-badge"
              ...
              {!report.subjectResults.length && (
                <tr>
                  ...
                  >
                    No course / subject records available for this selected
                    period.
                  </td>
                </tr>
              )}
        ...
        {(currentPeriodEndLine || nextPeriodLine || generatedDateValue) && (
          ...
          >
            ...
            {currentPeriodEndLine && nextPeriodLine && (
              ...
              >
                <span
                  className="university-transcript-timeline-marker"
                  style={{ borderColor: primary }}
                />
              </div>
            )}
            ...
            {generatedDateValue && (
              ...
              >
                ...
                <div style={strongValue}>Academic Transcript</div>
                <div
                  style={{
                    marginTop: 2,
                    fontSize: compact ? 8.8 : 9.7,
        ...
        >
          <span>Official academic transcript for {branding.schoolName}</span>
...
.university-transcript-template-page,
...
.university-transcript-frame-line {
  position: absolute;
  pointer-events: none;
  z-index: 0;
  border-radius: 3px;
}
.university-transcript-frame-outer { inset: 4.5mm; border: 1.5px solid; opacity: .65; }
.university-transcript-frame-inner { inset: 6.5mm; border: 1px solid #e5e7eb; }
.university-transcript-corner {
  position: absolute;
  z-index: 0;
  width: 17mm;
  height: 17mm;
...
.university-transcript-corner-tl { left: 4.5mm; top: 4.5mm; border-left: 3px solid; border-top: 3px solid; border-top-left-radius: 3px; }
.university-transcript-corner-tr { right: 4.5mm; top: 4.5mm; border-right: 3px solid; border-top: 3px solid; border-top-right-radius: 3px; }
.university-transcript-corner-bl { left: 4.5mm; bottom: 4.5mm; border-left: 3px solid; border-bottom: 3px solid; border-bottom-left-radius: 3px; }
.university-transcript-corner-br { right: 4.5mm; bottom: 4.5mm; border-right: 3px solid; border-bottom: 3px solid; border-bottom-right-radius: 3px; }
.university-transcript-grade-badge {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  min-width: 24px;
...
.university-transcript-timeline-marker {
  width: 12px;
  height: 12px;
  border-radius: 2px;
  border: 3px solid;
...
.university-transcript-timeline-marker::before,
.university-transcript-timeline-marker::after {
  content: "";
  position: absolute;
  top: 50%;
  width: 16px;
...
.university-transcript-timeline-marker::before { right: 100%; }
.university-transcript-timeline-marker::after { left: 100%; }
...
@media print {
  ...
  .university-transcript-template-page [data-report-color-block="true"] {
