...
/**
 * reports/student-report-templates/LetterHeadPremiumTemplate.tsx
 * ---------------------------------------------------------
 * ELEEVEON SCHOOLS — LETTERHEAD PREMIUM STUDENT REPORT TEMPLATE
 * ---------------------------------------------------------
 *
 * Visual direction:
 * - fixed A4/PDF sheet styled like premium institutional letterhead
 * - compact LetterHeadPremiumHeader that does not consume too much vertical space
 * - decorative guilloche/art-deco inspired border accents instead of bulky floral blocks
 * - refined student register panel, elegant results table and performance ribbon
 * - correspondence-style remarks, academic-period strip and official signature footer
 *
 * Data rule:
...
export default function LetterHeadPremiumTemplate({
  dataset,
  template,
  settings,
  compact = false,
    ...
    mergeStudentReportTemplateSettings(
      {
        ...DEFAULT_STUDENT_REPORT_TEMPLATE_SETTINGS,
        ...(settings || {}),
        templateCode: "letterhead_premium",
        layoutKey: "letterhead_premium",
        templateName: "LetterHead Premium",
  ...
  if (!dataset || !report || !header || !normalized) {
    return (
      <div className="src-empty-card letterhead-template-empty">
        <style>{css}</style>
  ...
  const upgradedPageStyle: React.CSSProperties = {
    ...
    margin: "0 auto 18px",
    padding: pagePadding,
    background: "linear-gradient(180deg, #fffdf8, #ffffff 34%, #fffdf8)",
    color: "#111",
  ...
  const tableStyles: ReturnType<typeof createReportTableStyles> = {
    ...
    th: {
      ...
      background: `linear-gradient(90deg, ${primary}, #111827)`,
      color: "#fff",
  ...
  const reportPage = (
    <section
      className="print-page report-page-break student-report-card-page src-a4-page letterhead-premium-template-page"
    ...
    >
      ...
      <div
        aria-hidden="true"
        className="letterhead-frame-line letterhead-frame-outer"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="letterhead-frame-line letterhead-frame-inner"
      />
      <div
        aria-hidden="true"
        className="letterhead-corner letterhead-corner-tl"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="letterhead-corner letterhead-corner-tr"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="letterhead-corner letterhead-corner-bl"
        style={{ borderColor: primary }}
      />
      <div
        aria-hidden="true"
        className="letterhead-corner letterhead-corner-br"
        style={{ borderColor: primary }}
      />

      <div style={{ position: "relative", zIndex: 1 }}>
        <LetterHeadPremiumHeader
          header={header}
          dataset={dataset}
          settings={resolvedSettings}
                ...
                <tr key={subject.classSubjectId}>
                  ...
                  {resolvedSettings.showGrade && (
                    <td style={{ ...tableStyles.td, textAlign: "center" }}>
                      <span
                        className="letterhead-grade-seal"
        ...
        {(currentPeriodEndLine || nextPeriodLine || generatedDateValue) && (
          ...
          >
            ...
            {currentPeriodEndLine && nextPeriodLine && (
              ...
              >
                <span
                  className="letterhead-timeline-medallion"
                  style={{ borderColor: primary }}
                />
              </div>
            )}
        ...
        <footer
          style={{
            marginTop: 8,
            borderTop: `1.5px double ${primary}`,
            paddingTop: 4,
            display: "flex",
        ...
        >
          <span>Official premium academic record • {branding.schoolName}</span>
...
.letterhead-premium-template-page,
...
.letterhead-frame-line {
  position: absolute;
  pointer-events: none;
  z-index: 0;
  border-radius: 8px;
}
.letterhead-frame-outer { inset: 4.5mm; border: 1.8px double; opacity: .75; }
.letterhead-frame-inner { inset: 7mm; border: 1px solid #e7ddbf; }
.letterhead-corner {
  position: absolute;
  z-index: 0;
  width: 18mm;
...
.letterhead-corner-tl { left: 4.5mm; top: 4.5mm; border-left: 3px double; border-top: 3px double; border-top-left-radius: 8px; }
.letterhead-corner-tr { right: 4.5mm; top: 4.5mm; border-right: 3px double; border-top: 3px double; border-top-right-radius: 8px; }
.letterhead-corner-bl { left: 4.5mm; bottom: 4.5mm; border-left: 3px double; border-bottom: 3px double; border-bottom-left-radius: 8px; }
.letterhead-corner-br { right: 4.5mm; bottom: 4.5mm; border-right: 3px double; border-bottom: 3px double; border-bottom-right-radius: 8px; }
.letterhead-grade-seal {
  display: inline-flex;
  align-items: center;
  justify-content: center;
  min-width: 24px;
...
.letterhead-timeline-medallion::before,
.letterhead-timeline-medallion::after {
  content: "";
  position: absolute;
  top: 50%;
  width: 16px;
...
.letterhead-timeline-medallion::before { right: 100%; }
.letterhead-timeline-medallion::after { left: 100%; }
...
@media print {
  ...
  .letterhead-premium-template-page [data-report-color-block="true"] {
