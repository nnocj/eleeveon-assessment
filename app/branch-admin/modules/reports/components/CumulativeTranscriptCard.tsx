...
function mergeTranscriptSettings(args: {
  ...
  return {
    ...
    layoutKey:
      args.template?.layoutKey || args.settings?.layoutKey || selectedCode,
    templateKey:
      args.template?.templateKey || args.settings?.templateKey || selectedCode,
    paperSize: args.template?.paperSize || args.settings?.paperSize || "A4",
    orientation:
      args.template?.orientation || args.settings?.orientation || "portrait",
    density:
      args.settings?.density ||
      args.template?.density ||
      (args.compact
        ? "compact"
        : DEFAULT_CUMULATIVE_TRANSCRIPT_SETTINGS.density),
  };
}
...
export default function CumulativeTranscriptCard({
  ...
  return (
    <div
      className={`ctc-preview-shell ${expanded ? "expanded" : ""} ${className}`}
    >
      <style>{css}</style>
      ...
      <div className="ctc-mobile-toolbar report-no-print">
        ...
        <div
          className="ctc-zoom-controls"
          aria-label="Cumulative transcript zoom controls"
        >
          <button
            type="button"
