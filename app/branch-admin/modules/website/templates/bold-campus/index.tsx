...
export function BoldCampusTemplate({ draft, schoolName, branchName, primaryColor, compact = false }: WebsiteTemplateRenderProps) {
  ...
  return (
    <div className={`actual-website-template bold-campus ${compact ? "compact" : ""}`} style={{ "--template-primary": primaryColor || "#7c3aed" } as React.CSSProperties}>
      ...
      <section className="bold-hero"><span>{branchName || "Main Campus"}</span><h3>{draft.tagline || "Learn boldly. Grow fully."}</h3><p>{draft.description || "A vibrant campus where learning, creativity and community come alive."}</p></section>
