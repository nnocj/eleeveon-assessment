...
export function ModernAcademyTemplate({ draft, schoolName, branchName, primaryColor, compact = false }: WebsiteTemplateRenderProps) {
  const name = draft.siteName || schoolName || "Your School";
  return (
    <div className={`actual-website-template modern-academy ${compact ? "compact" : ""}`} style={{ "--template-primary": primaryColor || "#2563eb" } as React.CSSProperties}>
      ...
      <section className="template-hero">
        <div><small>{branchName || "Main Campus"}</small><h3>{draft.tagline || "Learning today. Leading tomorrow."}</h3><p>{draft.description || "A modern school community helping every learner grow with confidence."}</p><div className="template-actions"><button>Explore School</button><button className="ghost">Apply Now</button></div></div>
