...
/**
 * app/branch-admin/modules/IdentityActivity.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON Identity Activity — PHASE 10
 *
...
const PRIMARY_TABLE = "identityActivityEvents";
const READ_ONLY = true;
const SOURCE_TABLES = [{"table": "identityActivityEvents", "label": "Activity"}];
const FIELD_CONFIG: FieldConfig[] = [];
const STATUS_KEYS = ["accepted", "pending", "denied", "failed", "duplicate"];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["identity_activity.view","identity_activity.read","identity_activity.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["identity_activity.manage","identity_activity.write"].includes(p)));
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"Activity Event updated successfully.":"Activity Event created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save Activity Event.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","Activity Event removed.")}catch(error){console.error(error);notify("error","Failed to remove Activity Event.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening Identity Activity..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view Identity Activity."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search identity activity..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>↯</i><h3>No records found</h3><p>No identity activity match the current search and filters.</p></section>:null}
