...
/**
 * app/branch-admin/modules/SchoolTransport.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON School Transport — PHASE 10
 *
...
const PRIMARY_TABLE = "schoolVehicles";
const READ_ONLY = false;
const SOURCE_TABLES = [{"table": "schoolVehicles", "label": "Vehicle"}, {"table": "transportRoutes", "label": "Route"}, {"table": "transportStops", "label": "Stop"}, {"table": "studentTransportAssignments", "label": "Assignment"}, {"table": "transportJourneys", "label": "Journey"}, {"table": "transportJourneyEvents", "label": "Journey Event"}];
const FIELD_CONFIG = [{"key": "name", "label": "Vehicle Name", "type": "text", "options": []}, {"key": "registrationNumber", "label": "Registration Number", "type": "text", "options": []}, {"key": "vehicleType", "label": "Vehicle Type", "type": "select", "options": ["bus", "van", "car", "other"]}, {"key": "capacity", "label": "Capacity", "type": "number", "options": []}, {"key": "driverName", "label": "Driver Name", "type": "text", "options": []}, {"key": "driverPhone", "label": "Driver Phone", "type": "text", "options": []}, {"key": "identityDeviceId", "label": "Identity Device ID", "type": "text", "options": []}, {"key": "active", "label": "Active", "type": "checkbox", "options": []}];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["school_transport.view","school_transport.read","school_transport.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["school_transport.manage","school_transport.write"].includes(p)));
 ...
 const displayRows=useMemo<DisplayRow[]>(()=>{const result:DisplayRow[]=[];SOURCE_TABLES.forEach((source:any)=>{(sourceRows[source.table]||[]).forEach((row:any)=>{const title=row.name || row.registrationNumber || 'Unnamed vehicle';const subtitle=`${humanize(row.vehicleType)} · ${row.registrationNumber || 'No registration'}`;const detail=row.driverName ? `${row.driverName}${row.capacity ? ` · ${row.capacity} seats` : ''}` : 'No driver assigned';const status=row.active === false ? "inactive" : "active";const time=Number(row.updatedAt||row.createdAt||0);result.push({id:idOf(row.id),kind:source.label,tableName:source.table,raw:row,title:String(title||source.label),subtitle:String(subtitle||""),detail:String(detail||""),status,time})})});return result.sort((a,b)=>b.time-a.time||a.title.localeCompare(b.title))},[sourceRows]);
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"School Vehicle updated successfully.":"School Vehicle created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save School Vehicle.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","School Vehicle removed.")}catch(error){console.error(error);notify("error","Failed to remove School Vehicle.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening School Transport..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view School Transport."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search school transport..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>▰</i><h3>No records found</h3><p>No school transport match the current search and filters.</p></section>:null}
