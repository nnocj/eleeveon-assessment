...
/**
 * app/branch-admin/modules/IdentityAccessPoints.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON Identity Access Points — PHASE 10
 *
...
const PRIMARY_TABLE = "identityAccessPoints";
const READ_ONLY = false;
const SOURCE_TABLES = [{"table": "identityAccessPoints", "label": "Access Point"}];
const FIELD_CONFIG = [{"key": "name", "label": "Name", "type": "text", "options": []}, {"key": "code", "label": "Code", "type": "text", "options": []}, {"key": "accessPointType", "label": "Type", "type": "select", "options": ["school_gate", "branch_gate", "classroom", "staff_office", "reception", "pickup_desk", "bus", "bus_stop", "assembly_point", "custom"]}, {"key": "locationLabel", "label": "Location", "type": "text", "options": []}, {"key": "latitude", "label": "Latitude", "type": "number", "options": []}, {"key": "longitude", "label": "Longitude", "type": "number", "options": []}, {"key": "allowedRadiusMeters", "label": "Allowed Radius (m)", "type": "number", "options": []}, {"key": "active", "label": "Active", "type": "checkbox", "options": []}];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["identity_access_points.view","identity_access_points.read","identity_access_points.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["identity_access_points.manage","identity_access_points.write"].includes(p)));
 ...
 const displayRows=useMemo<DisplayRow[]>(()=>{const result:DisplayRow[]=[];SOURCE_TABLES.forEach((source:any)=>{(sourceRows[source.table]||[]).forEach((row:any)=>{const title=row.name || 'Unnamed access point';const subtitle=`${humanize(row.accessPointType)} · ${row.code || 'No code'}`;const detail=row.locationLabel || (!isNaN(Number(row.latitude)) ? `${row.latitude}, ${row.longitude}` : 'No location');const status=row.active === false ? 'inactive' : 'active';const time=Number(row.updatedAt||row.updatedAt||row.createdAt||0);result.push({id:idOf(row.id),kind:source.label,tableName:source.table,raw:row,title:String(title||source.label),subtitle:String(subtitle||""),detail:String(detail||""),status,time})})});return result.sort((a,b)=>b.time-a.time||a.title.localeCompare(b.title))},[sourceRows]);
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"Access Point updated successfully.":"Access Point created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save Access Point.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","Access Point removed.")}catch(error){console.error(error);notify("error","Failed to remove Access Point.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening Identity Access Points..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view Identity Access Points."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search identity access points..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>⌖</i><h3>No records found</h3><p>No identity access points match the current search and filters.</p></section>:null}
