...
/**
 * app/branch-admin/modules/EmergencyRollCall.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON Emergency Roll Call — PHASE 10
 *
...
const PRIMARY_TABLE = "emergencyRollCallSessions";
const READ_ONLY = false;
const SOURCE_TABLES = [{"table": "emergencyRollCallSessions", "label": "Session"}, {"table": "emergencyRollCallEntries", "label": "Entry"}];
const FIELD_CONFIG = [{"key": "name", "label": "Session Name", "type": "text", "options": []}, {"key": "emergencyType", "label": "Emergency Type", "type": "select", "options": ["fire", "security", "medical", "weather", "drill", "other"]}, {"key": "status", "label": "Status", "type": "select", "options": ["active", "completed", "cancelled"]}, {"key": "startedAt", "label": "Started At", "type": "datetime-local", "options": []}, {"key": "endedAt", "label": "Ended At", "type": "datetime-local", "options": []}, {"key": "accessPointId", "label": "Assembly Point ID", "type": "text", "options": []}, {"key": "note", "label": "Note", "type": "textarea", "options": []}];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["emergency_roll_call.view","emergency_roll_call.read","emergency_roll_call.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["emergency_roll_call.manage","emergency_roll_call.write"].includes(p)));
 ...
 const displayRows=useMemo<DisplayRow[]>(()=>{const result:DisplayRow[]=[];SOURCE_TABLES.forEach((source:any)=>{(sourceRows[source.table]||[]).forEach((row:any)=>{const title=row.name || `${humanize(row.emergencyType)} roll call`;const subtitle=`${humanize(row.emergencyType)} · ${formatDate(row.startedAt)}`;const detail=row.accessPointId ? `Assembly point · ${row.accessPointId}` : 'No assembly point';const status=String(row.status || 'active'||"active");const time=Number(row.startedAt || row.updatedAt||row.updatedAt||row.createdAt||0);result.push({id:idOf(row.id),kind:source.label,tableName:source.table,raw:row,title:String(title||source.label),subtitle:String(subtitle||""),detail:String(detail||""),status,time})})});return result.sort((a,b)=>b.time-a.time||a.title.localeCompare(b.title))},[sourceRows]);
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"Emergency Session updated successfully.":"Emergency Session created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save Emergency Session.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","Emergency Session removed.")}catch(error){console.error(error);notify("error","Failed to remove Emergency Session.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening Emergency Roll Call..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view Emergency Roll Call."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search emergency roll call..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>⚠</i><h3>No records found</h3><p>No emergency roll call match the current search and filters.</p></section>:null}
