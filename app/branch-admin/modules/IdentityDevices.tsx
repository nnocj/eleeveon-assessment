...
/**
 * app/branch-admin/modules/IdentityDevices.tsx
 * --------------------------------------------------------------------------
 * ELEEVEON Identity Devices — PHASE 10
 *
...
const PRIMARY_TABLE = "identityDevices";
const READ_ONLY = false;
const SOURCE_TABLES = [{"table": "identityDevices", "label": "Device"}];
const FIELD_CONFIG = [{"key": "name", "label": "Name", "type": "text", "options": []}, {"key": "code", "label": "Code", "type": "text", "options": []}, {"key": "deviceType", "label": "Device Type", "type": "select", "options": ["phone", "tablet", "computer", "kiosk", "qr_scanner", "barcode_scanner", "fingerprint_scanner", "face_terminal", "nfc_reader", "rfid_reader", "vehicle_gateway", "gps_tracker", "api_gateway", "other"]}, {"key": "status", "label": "Status", "type": "select", "options": ["online", "offline", "maintenance", "disabled"]}, {"key": "serialNumber", "label": "Serial Number", "type": "text", "options": []}, {"key": "locationLabel", "label": "Location", "type": "text", "options": []}, {"key": "platform", "label": "Platform", "type": "text", "options": []}, {"key": "firmwareVersion", "label": "Firmware Version", "type": "text", "options": []}];
const STATUS_KEYS = ["online", "offline", "maintenance", "disabled"];
...
export default function Module(){
 ...
 const canView=roleCanView||permissions.some(p=>["identity_devices.view","identity_devices.read","identity_devices.manage","identity.view"].includes(p));
 const canEdit=!READ_ONLY&&(roleCanManage||permissions.some(p=>["identity_devices.manage","identity_devices.write"].includes(p)));
 ...
 const displayRows=useMemo<DisplayRow[]>(()=>{const result:DisplayRow[]=[];SOURCE_TABLES.forEach((source:any)=>{(sourceRows[source.table]||[]).forEach((row:any)=>{const title=row.name || 'Unnamed device';const subtitle=`${humanize(row.deviceType)} · ${row.code || 'No code'}`;const detail=row.locationLabel || row.serialNumber || 'No location';const status=String(row.status || (row.active === false ? 'disabled' : 'offline')||"active");const time=Number(row.lastSeenAt || row.updatedAt||row.updatedAt||row.createdAt||0);result.push({id:idOf(row.id),kind:source.label,tableName:source.table,raw:row,title:String(title||source.label),subtitle:String(subtitle||""),detail:String(detail||""),status,time})})});return result.sort((a,b)=>b.time-a.time||a.title.localeCompare(b.title))},[sourceRows]);
 ...
 async function save(){if(!canEdit)return notify("error","You do not have permission to edit this module.");try{setSaving(true);const payload:any={accountId,schoolId,branchId,isDeleted:false};FIELD_CONFIG.forEach((field:any)=>payload[field.key]=outputValue(field,form[field.key]));if(editingId)await updateLocal(PRIMARY_TABLE,editingId,payload);else await createLocal(PRIMARY_TABLE,payload);setModalOpen(false);await load();notify("success",editingId?"Device updated successfully.":"Device created successfully.")}catch(error){console.error(error);notify("error",error instanceof Error?error.message:"Failed to save Device.")}finally{setSaving(false)}}
 async function remove(row:DisplayRow){if(!canEdit||row.tableName!==PRIMARY_TABLE)return;try{await softDeleteLocal(PRIMARY_TABLE,row.id);setActionRow(null);await load();notify("success","Device removed.")}catch(error){console.error(error);notify("error","Failed to remove Device.")}}

 if(accountLoading||contextLoading||settingsLoading||loading)return <RouteState primary={primary} title="Opening Identity Devices..." text="Checking the active workspace and local records."/>;
 ...
 return <PermissionGate allowed={canView} fallback={<RouteState primary={primary} title="Access restricted" text="Your active membership does not allow you to view Identity Devices."/>}>
  <main className="ba-page" style={{"--ba-primary":primary} as React.CSSProperties}><style>{css}</style>
   ...
   <section className="ba-toolbar">
    ...
    <label className="search"><span>⌕</span><input value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search identity devices..."/></label>
   ...
   {!filteredRows.length?<section className="empty"><i>▣</i><h3>No records found</h3><p>No identity devices match the current search and filters.</p></section>:null}
