...
/**
 * app/parent/modules/Mychildreports.tsx
 * ---------------------------------------------------------
 * PARENT MY CHILD REPORTS MODULE
 * ---------------------------------------------------------
 *
 * No branch selector.
 *
...
export default function MyChildReports() {
  ...
  const canViewParentReports = role === "parent";
  ...
  const load = async () => {
    if (!authenticated || !accountId || !schoolId || !branchId || !canViewParentReports) {
      clearState();
    ...
    try {
      ...
      const matchedParent =
        scopedParents.find(
          (row: any) =>
            membershipParentId && idOf(row.id) === membershipParentId,
        ) ||
      ...
      const linkedStudentIds = new Set(
        scopedStudentParents
          .filter(
            (row: any) =>
              resolvedParentId && idOf(row.parentId) === resolvedParentId,
          )
      ...
      const linkedStudents = (scopedStudentsWithMedia as Student[]).filter(
        (student: any) => linkedStudentIds.has(idOf(student.id)),
      );

      const belongsToLinkedStudent = (row: any) =>
        linkedStudentIds.has(idOf(row.studentId));
      ...
      setParents(matchedParent ? [matchedParent] : []);
      ...
      setStudentParents(
        scopedStudentParents.filter((row: any) =>
          linkedStudentIds.has(idOf(row.studentId)),
        ),
      );
      setStudentEnrollments(
        enrollmentRows.filter(
          (row) => sameTenant(row) && belongsToLinkedStudent(row),
        ),
      );
      ...
      setAttendance(attendanceRows.filter((row) => sameTenant(row) && belongsToLinkedStudent(row)));
      setStudentAttendanceSummaries(
        (studentAttendanceSummaryRows as StudentAttendanceSummary[]).filter(
          (row: any) => sameTenant(row) && belongsToLinkedStudent(row),
        ),
      );
      setComputedResults(computedRows.filter((row) => sameTenant(row) && belongsToLinkedStudent(row)));
      setReportCards(scopedReportCards.filter(belongsToLinkedStudent));
  ...
  }, [authenticated, accountId, schoolId, branchId, dataRevision, canViewParentReports]);
  ...
  const renderActiveReport = () => {
    if (!hasCoreSetup) {
      return (
        <Empty
          icon="📄"
          title="No child report is available yet"
          text="Your school has not published enough academic data to build this child report yet."
        />
      );
    }
  ...
  if (accountLoading || contextLoading || settingsLoading || pageLoading) {
    return (
      <State
        primary={primary}
        title="Opening My Child Reports..."
        text="Checking your linked children, academic records, report templates and attendance."
      />
    );
  }
  ...
  if (!authenticated || !accountId) {
    return (
      <State
        ...
        text="You must sign in before opening child reports."
      />
    );
  }

  if (!canViewParentReports) {
    return (
      <State
        primary={primary}
  ...
  if (!membershipParentId && parents.length === 0 && !pageLoading) {
    return (
      <State
        primary={primary}
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          ...
          <p>
            Child reports are generated inside the selected branch-admin
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="Child report search and actions"
      >
        <label className="ba-search">
          ...
          <input
            ...
            placeholder="Search child reports..."
            aria-label="Search child reports"
          />
        </label>
        ...
        <div className="ba-form compact">
          <label>
            ...
            >
              ...
              <option value="class-reports">All Children Reports</option>
            </select>
          </label>
...
/* My Child Reports golden additions */
