...
/**
 * app/parent/modules/MyChildCumulativeRecord.tsx
 * ---------------------------------------------------------
 * PARENT PORTAL — MY CHILD'S CUMULATIVE RECORD
 * ---------------------------------------------------------
 *
...
export default function MyChildCumulativeRecord() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const matchedParent =
        scopedParents.find(
          (row: any) =>
            membershipParentId && sameId(row.id, membershipParentId),
        ) ||
        scopedParents.find((row: any) => {
      ...
      const resolvedParentId = firstLocalId(
        membershipParentId,
        matchedParent?.id,
      );
      const linkedStudentIds = new Set<string>(
        scopedStudentParents
          .filter(
            (row: any) =>
              resolvedParentId && sameId(row.parentId, resolvedParentId),
          )
          .map((row) => idOf(row.studentId))
          .filter(Boolean),
      );
      const portalStudents = (studentsWithMedia as Student[]).filter((row) =>
        linkedStudentIds.has(idOf(row.id)),
      );
      const portalStudentParents = scopedStudentParents.filter((row) =>
        linkedStudentIds.has(idOf(row.studentId)),
      );
      const portalParents = matchedParent ? [matchedParent] : [];
      ...
      setStudentReportSnapshots(
        snapshotRows.filter((row) => {
          if (!linkedStudentIds.has(idOf(row.studentId))) return false;
      ...
      setFilters((prev) => {
        const currentIsLinked =
          !!prev.studentId && linkedStudentIds.has(idOf(prev.studentId));
        return {
          ...prev,
          studentId:
            currentIsLinked
              ? prev.studentId
              : idOf(portalStudents[0]?.id) || undefined,
        };
      });
  ...
  if (accountLoading || contextLoading || settingsLoading || loading) {
    return (
      <State
        primary={primary}
        title="Opening My Child's Cumulative Record..."
        text="Checking your linked children's historical reports, promotions and academic progression."
      />
    );
  }
  ...
  if (!authenticated || !accountId) {
    return (
      <State
        ...
        text="You must sign in before viewing your child's cumulative record."
      />
    );
  }
  ...
  if (!canViewCumulativeRecords) {
    return (
      <State
        ...
        text="Open this page from an active parent-role workspace."
      />
    );
  }

  if (!membershipParentId && parents.length === 0) {
    return (
      <State
        primary={primary}
        title="Parent profile not linked"
        text="The active parent membership needs a parentId, parentLocalId, or matching parent contact."
      />
    );
  }
    ...
    return (
      ...
      >
        ...
        <section className="ba-state">
          <h2>Parent workspace unavailable</h2>
          <p>
            Your child's cumulative record requires an active school and branch workspace.
          </p>
          <button
            type="button"
  ...
  return (
    ...
    >
      ...
      <section
        ...
        aria-label="My child's cumulative record search and actions"
      >
        <label className="ba-search">
          ...
          <input
            ...
            placeholder="Search my child's cumulative record..."
            aria-label="Search my child's cumulative record"
          />
        </label>
      ...
      <section className="ba-sheet">
        <div className="ba-sheet-head">
          <div>
            <h2>Filters</h2>
            <p>
              Choose a linked child, historical period, subject, or record type. School and branch stay locked.
            </p>
          </div>
      ...
      <section className="ba-sheet small">
        ...
        <div className="ba-menu-list">
          ...
          <button type="button" onClick={onRefresh}>
            <span>↻</span>
            <b>Refresh</b>
            <small>Reload my child's cumulative records</small>
          </button>
        </div>
      </section>
    </div>
