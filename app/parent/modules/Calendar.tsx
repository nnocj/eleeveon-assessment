...
/**
 * app/parent/modules/Calendar.tsx
 * ---------------------------------------------------------
...
export default function Calendar() {
    ...
    try {
      ...
      setEvents((rows as AnyRow[]).filter((event) =>
        ...
        (
          ...
          event.eventType === "fee_deadline" ||
          event.eventType === "parent_teacher_meeting" ||
          childIds.has(Number(event.studentLocalId)) ||
          childIds.has(Number(event.scopeId)) ||
          ctx.children.some((child) => Number(child.classId || child.currentClassId) === Number(event.classId))
        )
      ));
    } finally {
      setLoading(false);
  ...
  const filtered = useMemo(() => {
    ...
    return events.filter((event) => {
      if (childId !== "all") {
        ...
        const matchesChild =
          Number(event.studentLocalId) === Number(child?.id) ||
          Number(event.classId) === Number(child?.classId || child?.currentClassId) ||
          Number(event.scopeId) === Number(child?.id);
  ...
  const summary = useMemo(() => {
    ...
    return {
      ...
      meetings: events.filter((event) => event.eventType === "parent_teacher_meeting" || event.eventType === "meeting").length,
      feeDeadlines: events.filter((event) => event.eventType === "fee_deadline").length,
      children: children.length,
    };
  }, [events, children]);

  if (loading || accountLoading || settingsLoading || contextLoading) {
    return <main className="pp-page" style={{"--pp-primary": primary} as React.CSSProperties}><style>{css}</style><section className="pp-state"><h2>Opening calendar...</h2><p>Loading child events, meetings and deadlines.</p></section></main>;
  }
  ...
  return (
    <main className="pp-page" style={{"--pp-primary": primary} as React.CSSProperties}>
      ...
      <section className="pp-hero">
        <div className="pp-hero-left"><div className="pp-icon">📅</div><div className="pp-title"><p>Parent Calendar</p><h2>Calendar</h2><span>{rowName(parent || {})} · {activeBranch?.name || "Assigned branch"}</span></div></div>
      ...
      <section className="pp-summary-grid">
        ...
        <SummaryCard label="Fee Deadlines" value={summary.feeDeadlines} icon="💳" warning={summary.feeDeadlines > 0} />
      ...
      {view === "analytics" && (
        <section className="pp-section pp-breakdown-grid">
          {["meeting","parent_teacher_meeting","fee_deadline","exam","assessment","holiday"].map((kind) => {
            const count = events.filter((event) => event.eventType === kind).length;
            const pct = events.length ? Math.round((count / events.length) * 100) : 0;
            return <article key={kind} className="pp-breakdown"><strong>{kind.replaceAll("_"," ")}</strong><div className="pp-bar"><div style={{width:`${pct}%`}} /></div><div className="pp-chip-row"><Chip tone="blue">{count}</Chip><Chip>{pct}%</Chip></div></article>;
          })}
        </section>
      )}

      {view === "table" && (
        <section className="pp-panel"><div className="pp-table-wrap"><table className="pp-table"><thead><tr><th>Event</th><th>Type</th><th>Starts</th><th>Ends</th><th>Location</th><th>Status</th></tr></thead><tbody>
          {filtered.map((event) => <tr key={event.id}><td><strong>{event.title}</strong><br/><span>{event.description || "-"}</span></td><td>{event.eventType}</td><td>{dateLabel(event.startAt)}</td><td>{dateLabel(event.endAt)}</td><td>{event.location || "-"}</td><td><Chip tone="green">{event.status || "scheduled"}</Chip></td></tr>)}
      ...
      {view === "cards" && (
        <section className="pp-section"><div className="pp-list">
          {filtered.map((event) => <article key={event.id} className="pp-card"><div className="pp-card-top"><div className="pp-avatar">📅</div><div className="pp-card-main"><h3>{event.title}</h3><p>{dateLabel(event.startAt)} · {event.location || "No location"}</p><div className="pp-chip-row"><Chip tone="blue">{event.eventType}</Chip><Chip tone={event.priority === "urgent" ? "red" : event.priority === "high" ? "orange" : "gray"}>{event.priority || "normal"}</Chip></div></div></div>{event.description && <p className="pp-message">{event.description}</p>}</article>)}
