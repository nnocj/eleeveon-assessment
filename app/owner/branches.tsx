...
/**
 * app/owner/branches.tsx
 * Eleeveon Owner Branches V2.
...
export default function OwnerBranchesPage() {
  ...
  const requireAccount = () => {
    if (!authenticated || !selectedAccountId) {
      showToast("error", "Sign in before managing owner branches.");
      return false;
    }
    return true;
  };
  ...
  const load = async () => {
    ...
    try {
      ...
      console.error(error);
      clearData();
      showToast("error", "Failed to load owner branches.");
    } finally {
      setLoading(false);
    }
  };
  ...
  if (accountLoading || settingsLoading || loading) {
    return <State primary={primary} title="Opening Branches..." text="Checking owner account, local branch records, school assignment, and media assets." />;
  }

  if (!authenticated || !selectedAccountId) {
    return <State primary={primary} title="Redirecting to login..." text="You must sign in before managing owner branches." />;
  }
    ...
    <main className="ba-page" style={{ "--ba-primary": primary } as React.CSSProperties}>
      ...
      {viewMode === "cards" && (
        <section className="ba-list">
          ...
          {!filteredRows.length && (
            <Empty icon="🏛️" title="No branches found" text="Add owner branch records, connect logo and banner images, and assign each branch to a school." />
          )}
        </section>
      )}
